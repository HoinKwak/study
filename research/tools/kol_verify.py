...
def check_token(name, th, m, bad):
    ...
    if dpos >= 0 and m["dliq_pct"] is not None:
        ...
        for mo in re.finditer(r"\(?([+-][\d.]+)%", seg):
            ...
            #   ⚠️확장(9/6 11:00Z): %와 '에서' 사이에 짧은 어구가 끼는 형태가 훨씬 흔하다
            #     ("+4.6%**의 유입**에서 -2.5%로 유출 전환") — 15종에서 오탐이 났다.
            #     문장 끝·다른 %를 넘지 않는 짧은 다리만 허용한다.
            #     '에 이어'도 같은 전이다("-2.0%에 이어 -1.1%로 유출 지속" — BULLSHIT).
            if re.match(r"\s*(?:(?:[^%.]{0,10}?(?:에서|에\s*이어))|→|->)\s*[^%]{0,12}%",
                        seg[mo.end():]):
                continue
    ...
    for k in ("h1", "h6", "h24"):
        for hm in re.finditer(rf"{k}\s*([+-]?[\d.]+)%", th):
            # ⚠️(9/6 03:00Z) 과거 회차 표지가 **값 앞**에 오는 형태를 못 걸렀다 —
            #   "직전회차복원됐던h1+0.72%가 …재결측"은 직전 회차 값 인용인데 이번
            #   회차 주장으로 읽어 '결측인데 인용했다'는 오탐을 냈다(MANEKI).
            #   유동성Δ 쪽은 표지가 값 **뒤**에 오는 형태만 처리하고 있었다.
            #   사이에 숫자·%가 끼면 그 표지는 다른 값에 붙은 것이므로 계속 검사한다.
            #   ⚠️범위 축소: `[^%\d]{0,10}`은 절 경계를 넘어 "직전결측) — h1-0.87%"의
            #     **이번 회차 값**까지 건너뛰었다(범위 점검에서 발각 — 주입 3종은
            #     전부 통과했는데도 검출력이 조용히 깎이고 있었다). 표지가 값에
            #     직접 붙은 경우만 과거로 본다(괄호·줄표·쉼표가 끼면 다른 절이다).
            if re.search(r"(?:직전(?:\s*회차)?|전회|과거|\d+\s*회차\s*전)"
                         r"[^%\d)\]·,—\-]{0,10}$", th[:hm.start()]):
                continue
            # ⚠️검출 공백(9/6 23:00Z): 유동성·회전율·풀수·회차는 A→B 전이를 처리하는데
            #   **h 계열과 vol24만 빠져 있었다** — "h24 -28.59%→-15.98%"에서 앞의 직전
            #   값을 이번 회차 주장으로 읽었다. 오탐일 뿐 아니라 **뒤쪽(진짜 주장)이
            #   무검사로 지나가는 검출 공백**이다. 이번 회차에 기준선 사정 때문에
            #   에이전트가 전이 표기를 많이 쓰면서 드러났다.
            nxt = re.match(r"\s*(?:→|->)\s*([+-]?[\d.]+)%", th[hm.end():])
            v = float(nxt.group(1) if nxt else hm.group(1))
            if m[k] is None:
...
def main() -> int:
    ...
    # ⚠️오탐 수정(9/6 23:00Z): '42종중 **최고령**'(풀나이)의 '최고'가 vol24 최상급으로
    #   잘려 읽혔다(BARRON). 최상급 낱말 뒤에 다른 글자가 붙어 **별개 낱말**이 되는
    #   형태는 제외한다('령'·'점'·'치' 등).
    SUP = re.compile(r"42종중\s*(?:이번회차)?\s*"
                     r"(?P<dir>증가|감소|유입|유출|변동|급증|급감)?\s*"
                     r"(?P<sup>최대|최고|최저|최소|최하)(?!위권|위|령|점|치)")
    ...
    # ⚠️(9/6 05:00Z) **자리를 남에게 넘겼다**는 서술도 이탈 표현인데 안 걸렸다 —
    #   "h24완화(42종중최대자리CC에넘김)"에서 1B가 최대를 주장한 것으로 읽어 오탐이
    #   났다(실제 최대는 CC). 인계 동사(넘김·넘겼·넘어갔·내줬)를 추가하고, 사이에
    #   **새 보유자 이름**이 끼므로 짧은 다리를 허용한다.
    _EXIT = re.compile(r"^\s*(?:지위|자리|권)?\s*(?:에서|를|을)?\s*(?:벗어|이탈|내주|물러)"
                       r"|^\s*(?:이|가)?\s*아니"
                       r"|^\s*(?:지위|자리|권)?\s*[가-힣A-Za-z0-9]{0,12}?"
                       r"(?:에게|에|은|는|로)?\s*(?:넘김|넘겼|넘어갔|넘어감|내줬|내주었)")
    ...
    def scan(text: str, subject: str | None, where: str) -> None:
        for mo in SUP.finditer(text):
            ...
            # ⚠️절 경계에서 끊는다(9/5 21:00Z): 앞 창이 **다른 절**까지 넘어가 델타 표지를
            #   끌어왔다 — "유동성유출전환(-1.8%). ⚠️h24여전히42종중최소"에서 앞 절의 '유출'을
            #   집어 h24 **수준** 최소 주장을 유동성 감소 최대 주장으로 판정했다(DPG 오탐 2건).
            pre_w = re.split(r"[.!?)·]|⚠️", pre_w)[-1]
            ...
            # ⚠️오탐 수정(9/6 23:00Z): "42종중 최대 **수준**을 유지"는 명시적인 **수준**
            #   주장인데, 같은 표 칸 뒤쪽의 다른 % 값(h24 +26.14%)이 델타 표지로 세어져
            #   유동성 변동 최대 주장으로 판정됐다(CATE). '수준'이 바로 뒤에 오면 델타 판정을
            #   하지 않는다 — 이 낱말은 델타와 함께 쓰이지 않는다.
            _level_word = bool(re.match(r"\s*수준", after))
            is_delta = (not _level_word) and (bool(_inner) or bool(re.match(r"\s*폭?(?:으로|만큼)?\s*(?:변동|증가|증분|유입|유출|감소|급증|급감|빠|줄|늘|불)", after))
                        ...
                        # h 계열은 유동성 델타와 비교 자체가 무의미하므로 앞 창 표지도 쓰지 않는다.
                        or (_m_early not in ("h1", "h6", "h24")
                            and bool(re.search(r"(변동(?!\s*없)|증가|증분|유입|유출|감소|급증|급감)", pre_w))))
            if is_delta:
