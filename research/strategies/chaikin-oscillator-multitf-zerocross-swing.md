...
- **백테스트 근거**: **BTC/USDT 2021~2024, 4H 제로라인 크로스 단독** = 승률 48% / PF 1.24 / 거래수 82건.
  **일봉 차이킨 오실레이터 방향 필터 추가**(일봉 양수일 때만 4h 롱, 음수일 때만 4h 숏) 시 승률 59% / PF 1.72 /
  거래수 38건(필터로 진입 빈도 감소, 질 향상). 표본기간 3년, 단일 종목(BTC)만 확인돼 **다른 종목 일반화 여부·
  수수료 반영 여부는 출처에 명시 안 됨 → 우리 프레임워크로 재검증 필수**.
