...
OOS 명목 n=133, **고유 진입일=114**(명목:고유=1.17). 2종목 이상 동시진입일 **18일**,
**7종목 전부 동시진입일 0건**. **이벤트(날짜) 단위 재집계**: n=114, PF=1.456, **t=+1.410**
(여전히 비유의) — 의사반복으로 인한 t 부풀림 우려는 크지 않고, 애초에 봉 단위 t 자체가
비유의라 결론 불변.

> ### ⚠️ 리뷰 정정 (backtest-reviewer) — 초판의 "전면적 매크로 클러스터는 아님"은 **오판**
>
> 초판은 **같은 캘린더일** 동시진입만 검사해 "전면적 매크로 클러스터는 아니다"라고 결론냈다.
> 그러나 종목별 EMA 눌림목 타이밍이 며칠씩 어긋나므로 **같은 매크로 이벤트라도 같은 날에는
> 절대 겹치지 않는다** — 검사 단위가 너무 좁아 실재하는 클러스터를 놓쳤다.
> 리뷰어가 **5일 롤링 윈도우**로 재검사한 결과:
>
> ```
> 2026-01-19~22 진입 · 2026-02-06~08 청산 클러스터 (6건, 5/7종목)
>   BTC 2026-01-21 20:00 → +598.98 (chandelier)
>   BNB 2026-01-21 16:00 → +536.14 (chandelier)
>   SOL 2026-01-20 16:00 → +460.80 (chandelier)
>   XRP 2026-01-19 04:00 → +336.78 (chandelier)
>   ADA 2026-01-19 00:00 → +279.51 (chandelier)
>   BTC 2026-01-20 08:00 → -110.97
>   합산 +2,101.2 = OOS 총순익(+2,933.4)의 71.6%
>
> 이 6건 제외 시: n=127, PF=1.122, t=+0.526
> ```
>
> 데이터 오류가 아니라 **실제 대형 시장 이벤트**임을 BTC 1d klines로 확인했다 —
> BTC가 2026-01-17 ~$95K → 2026-02-05 ~$62.9K로 **약 34% 폭락한 뒤 2026-02-06 하루 만에 +12% 반등**.
> 이 국면(2025Q4~2026Q1)은 **완전히 다른 백테스트(BTC-금 로테이션, SUSPECT 판정)에서도 독립적으로
> "BTC 매크로 추세 3건" 중 하나로 이미 기록된 바로 그 붕괴장**이다. 서로 다른 두 전략이 같은
> 매크로 이벤트를 각각 우연히 포착한 것으로 교차 확인된다.
>
> **더 결정적**: 게이트없음(ungated) 대조군도 이 사건에 완전히 노출된다.
>
> ```
> ungated 전체 OOS      : n=380, PF=1.055, net=+1,182.8
> ungated (클러스터 제외): n=362, PF=0.968, net=  -653.0   ← 순손실로 전환
> ```
>
> 즉 **게이트없음 대조군의 OOS 엣지(PF 1.055>1) 전체가 이 단일 사건 하나에서 나온 것**이고,
> 사건을 빼면 순손실이다. 게이트 있음 쪽은 사건 제외 후에도 소폭 양(+832.2, PF 1.122)이라
> §11의 "게이트가 대조군보다 우수"(조건 a)는 방향이 유지되지만, **두 대조군 모두의 OOS 표면
> 수치가 하나의 비반복적 시장 국면에 크게 오염돼 있다**.
>
> **정정된 결론**: 이 전략의 OOS 표면 성과는 실질적으로 **하나의 비반복적 매크로 국면의 산물**이다.
> 초판의 "표본은 충분하나 통계적으로 불확실"이라는 조심스러운 프레이밍보다 이 진단이 더 명확하며,
> **FAIL 판정을 뒤집지 않고 오히려 강하게 보강한다.**
...
정합적으로 **단일 종목(SOL) 의존이 이 전략 순익의 절반 이상을 설명**한다.

> **⚠️ 리뷰 정정 (backtest-reviewer)**: "SOL 의존"은 **부분적 오귀인(misattribution)**이다.
> SOL의 OOS 순익(1,602.5) 중 **460.8(29%)이 §4에서 규명된 공유 매크로 이벤트에서 나온 것**이므로,
> "SOL만의 고유 정보력"이 아니라 **여러 종목이 공유한 하나의 이벤트를 SOL도 함께 포착한 결과**가
> 섞여 있다. 다만 4.5년 전체 평균 종목간 신호 상관은 낮으므로(아래) 과거 BTC-금 SUSPECT 건처럼
> "전 구간에 걸친 체계적 착시"는 아니고, **한 번의 위기 국면에 국한된 국지적 착시**다. IS 에서도 종목별
...
## 부록: backtest-reviewer 감사 결과 — **VALID** (FAIL 유지, 근거는 더 강해짐)

### 최우선 쟁점 — 사전 폐기조건 (b)의 문자 그대로 대조
표면 OOS PF 1.423이 통과선 1.3을 넘었는데 FAIL로 간 사례라, **스펙 원문의 폐기조건을 그대로
적용했는지**가 최우선 검증 대상이었다. 리뷰어가 스펙 파일 39행을 직접 확인한 결과:

> **사전 폐기조건(사후 변경 금지)**: (a) … (b) **OOS PF(R) < 1.15 또는 t 비유의** (c) … (d) …

리포트가 인용한 **"또는(OR)" 결합은 스펙 원문과 글자 그대로 일치**한다. 리포트가 지어낸 것이 아니다.
- `t ≥ 1.96(p<0.05)` 관행 기준도 임의 신설이 아니라 프로젝트 전반에서 일관되게 쓰인다
  (`grep -rl "1.96" research/backtests/*.md` → 20개 이상 리포트에서 확인).
- (d)의 "메이저" 판정도 `src/crypto_trader/config/settings.py`의 `major_bases` 기본값에 SOL이
  포함됨을 직접 확인해 정확하다.

→ **사후 기준 조작 정황 없음.** 과거에 리뷰어가 FAIL→HOLD로 정정했던 "사후 기준 신설" 사례와는
다른 유형이고, "표면 PF가 통과선을 넘어도 사전 폐기조건에 걸리면 FAIL"이라는 **지지된 선례와 같은 구조**다.

### 재현 범위
- **`analyze.py` 재실행 → `analyze_out.txt`와 바이트 단위 완전 일치**(`diff` IDENTICAL).
  18개 섹션 전체(셔플·부트스트랩·룩어헤드·회계정합 포함) 재현.
- **`per_symbol.py` 재실행 → §19 종목별 표와 완전 일치.**
- 룩어헤드 3개 절단점 재실행도 byte 일치, `gate_on` 불일치 0건·`rho` 오차 0.
- 재현 실패 항목 없음.

### ⚠️ 리뷰어 신규 발견 (본문 §4·§19에 정정 반영 완료)
**다일간(3~5일) 매크로 클러스터** — 초판이 같은 캘린더일만 검사해 놓친 것을 5일 롤링 윈도우로
규명. **OOS 순익의 71.6%가 2026-01-19~22 진입·2026-02-06~08 청산의 단일 이벤트**(5/7종목·6건)에서
발생하며, 이 6건 제외 시 n=127·PF 1.122·t=+0.526으로 붕괴한다. 실제 시장 이벤트(BTC 34% 폭락 후
하루 +12% 반등)임을 1d klines로 확인했고, **다른 백테스트(BTC-금 로테이션, SUSPECT)에서 독립적으로
기록된 "2025Q4~2026Q1 붕괴장"과 동일 국면**임이 교차 확인된다.
**게이트없음 대조군의 OOS 엣지 전체(+1,182.8)도 이 사건 하나에서 나오며 제외 시 순손실(-653.0)**이다.

### 리뷰어가 직접 계산한 종목 간 신호 상관 (스펙이 "필수 보고"라 했으나 리포트가 누락)
- `gate_on` 일별 지표 pairwise 상관 평균 **+0.070**
- 실제 진입신호 일별 지표 pairwise 상관 평균 **+0.040~0.036**
- 스펙 사전 예상(0.4~0.6)보다 **훨씬 낮아** 표면적으로는 종목 간 독립으로 보이지만,
  위 매크로 클러스터가 보여주듯 **위기 국면의 꼬리 상관은 별개**다.

### 항목별 판정
| 항목 | 결과 |
|---|---|
| 단위(ns/ms/us) | PASS — klines·funding 모두 13자리 ms 정수 epoch를 `unit="ms"`로 파싱. **`metrics.create_time`(문자열)은 애초에 미파싱**(파일명 날짜 + 정렬 확인된 행 순서로만 처리)이라 ms/us 함정에 구조적으로 미노출 |
| 룩어헤드 | PASS — 3개 절단점 재현 byte 일치, `avail.index+1day` 시프트 논리 직접 검토 |
| 표본크기 | PASS — OOS n=133, 종목당 15~23건 |
| OOS 분리 | PASS — `IS+OOS==FULL==263` 정확 일치, IS_END 23:59:59 명시 |
| 유니버스/선택 편중 | **CONCERN(리포트 과소평가)** — 위 매크로 클러스터 |
| 수수료·슬리피지 | PASS — gross 1.466 vs net 1.423, 비용이 결론을 좌우하지 않음 |
| 워밍업·지표 | PASS — EMA/ATR/corr 워밍업 충분, `pandas.corr()` 사용(`np.corrcoef` 없음) |
| 반전 대조군 버그 | PASS — `is_long = entry_dir == "direction"`으로 최종방향만 참조, 원신호 플래그 미참조 |
| zero_hold_frac 유효성 | PASS — 진입 후 최소 1봉 뒤에야 exit 체크하는 구조임을 코드로 확인, 트리비얼 지표 주장 타당 |
| base ⊆ pool | PASS — 72/133(54%) 중복을 리포트가 스스로 발견해 독립 Welch(p=0.099) 병행, 신규 규칙 올바르게 적용 |
| 계좌소진 | PASS — §11·§14가 R-배수 병기(1.424 vs 1.053/1.010) |
| 재현성 | PASS(최고 수준) — byte 단위 일치. 단 metrics 재다운로드 시나리오는 캐시가 살아 있어 미검증(한계로 남김) |

### 결론 및 권고
- **VALID · FAIL 유지 · 라이브 미반영에 강하게 동의.** 판단불가로 내릴 근거 없음
  (표본 자체는 충분하고, 그 안의 유효 정보가 거의 없음이 오히려 명확해졌다).
- **⚠️ 파이프라인 권고(전 백테스트 해당)**: 스펙이 요구하는 "종목 간 신호 상관 사전 명시·보고"를
  **연간 평균 상관만이 아니라 위기 국면(꼬리) 상관까지 별도로 점검**하도록 요건을 강화할 것.
  금융에서 잘 알려진 **"평시엔 낮고 위기엔 급등하는 상관"** 이 이번 사례의 핵심 함정이었다.
- **⚠️ de-clustering 규칙 강화**: 같은 캘린더일 동시진입 검사만으로는 부족하다.
  **종목별 진입 타이밍이 어긋나는 전략(EMA 눌림목 등)은 반드시 3~5일 롤링 윈도우로 재검사**할 것.
