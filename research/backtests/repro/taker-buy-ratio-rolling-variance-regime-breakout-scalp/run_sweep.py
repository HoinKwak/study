"""파라미터 스윕: tvr_pctile_th·donchian_lb·atr_trail_mult·atr_stop_mult·max_hold_bars.
tvr_window 은 신호 자체를 재계산해야 하므로 common.with_tvr_window() 로 별도 처리."""
import pickle
import time
...
for th in (20, 25, 30, 35, 40):
    variants.append(("tvr_pctile_th", th, dict(tvr_pctile_th=float(th))))
...
for name, val, kw in variants:
    ...
    cfg = engine.RunConfig(gate="tvr_lo", fee_on=True, **kw)
...
# tvr_window 스윕(신호 재계산)
print("\n=== tvr_window 스윕(신호 재계산 필요) ===")
for w in (16, 20, 24, 30, 36):
    use_sigs = {s: common.with_tvr_window(sig, w) for s, sig in sigs.items()}
    cfg = engine.RunConfig(gate="tvr_lo", fee_on=True)
    trades = engine.run_all(use_sigs, cfg)
    all_trades = [t for lst in trades.values() for t in lst]
    df = su.trades_df(all_trades)
    _, oos_df, _ = su.split_is_oos(df)
    s = su.summary(oos_df, f"tvr_window={w}")
    print(su.print_summary(s))
