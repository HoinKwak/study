"""테이커 매수비율 롤링 분산(tvr_std) 게이트 + Donchian 브레이크아웃 — 백테스트 엔진.

신호(15m, tvr_std 게이트+Donchian 이탈) → 15m 진입/청산:
  - 15m 봉 i 종가 시점에: close(i) 가 직전 20봉(자기 제외) Donchian 상단/하단을 이탈
    AND tvr_std_pctile(i) <= 30(응축 게이트) AND 24봉(tvr_window) 롤링 테이커매수비율 평균이
    이탈 방향과 일치(롱: 평균>1.0, 숏: 평균<1.0) → 방향 확정(모두 i 시점까지의 데이터만 사용,
    룩어헤드 없음).
  ...
  - gate:
      tvr_lo  = 스펙 기본(tvr_std_pctile<=30 AND 방향확인 일치)
      none    = 게이트 없는 순수 Donchian(20) 브레이크아웃(tvr_std·방향확인 조건 전부 제거,
                사전 폐기조건 (e)의 문언 그대로 "게이트 없는 순수 Donchian 브레이크아웃 대조군")
      tvr_hi  = 게이트 반대방향(tvr_std_pctile>=70, 확장/출렁임 국면) — 방향 조건은 tvr_lo 와
                동일. 스펙 본문의 "확장 국면 브레이크아웃은 방향지속성 낮음" 가설의 직접 대조.
      rv_lo   = 신호원 교체(tvr_std 대신 가격 실현변동성 rv15 의 저분산 30%) — 사전 폐기조건 (b)
                "게이트 없음(RV 스퀴즈 대조군)"·전제일관성 절의 "tvr_std 게이트=가격 저변동성
                게이트" 의심 직접 검증
      reverse = tvr_lo 와 같은 신호 정의에서 최종 방향만 반전(SL/TP 는 진입가 기준 대칭 재배치,
                청산 로직은 최종 방향 변수 참조 — ATR 기반이라 방향 비대칭 없음, 절대레벨 재사용
                버그 클래스 없음)
"""
...
sys.path.insert(0, os.environ.get(
    "TBRV_REPO_SRC", "/home/user/study/.claude/worktrees/agent-a09efab3ace244b40/src"))
...
class TradeRec:
    ...
    gate: str                # tvr_lo|none|tvr_hi|rv_lo|reverse
...
@dataclass
class RunConfig:
    tvr_pctile_th: float = common.TVR_PCTILE_TH
    ...
    gate: str = "tvr_lo"        # tvr_lo|none|tvr_hi|rv_lo|reverse
    fee_on: bool = True
...
def run_symbol(symbol: str, sig: common.Signals, cfg: RunConfig, settings, risk: RiskManager
    ...
    if cfg.gate == "tvr_lo":
        gate_pctile = sig.tvr_std_pctile.to_numpy(float)
        gate_ok = gate_pctile <= cfg.tvr_pctile_th
        need_dir_confirm = True
    elif cfg.gate == "tvr_hi":
        gate_pctile = sig.tvr_std_pctile.to_numpy(float)
        gate_ok = gate_pctile >= (100.0 - cfg.tvr_pctile_th)
        need_dir_confirm = True
    elif cfg.gate == "rv_lo":
        gate_pctile = sig.rv15_pctile.to_numpy(float)
        gate_ok = gate_pctile <= cfg.tvr_pctile_th
        need_dir_confirm = True
    elif cfg.gate == "none":
        gate_pctile = sig.tvr_std_pctile.to_numpy(float)  # 기록용(게이트 미적용)
    ...
    for i in range(1, n15 - 1):
        ...
        if need_dir_confirm:
            ...
            if not np.isfinite(tm):
                continue
            if raw_direction == "long" and not (tm > 1.0):
                continue
            if raw_direction == "short" and not (tm < 1.0):
                continue
        ...
        # ⚠️진입수수료 반영(과거 여러 라운드에서 발견된 버그 — 진입 fee0 이 pnl 에서 누락되던
        # 것을 여기서는 애초에 raw - fee0 - fee1 로 계산해 방지).
