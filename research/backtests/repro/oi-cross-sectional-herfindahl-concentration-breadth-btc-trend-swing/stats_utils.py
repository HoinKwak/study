...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    rows = []
    for t in trades:
        rows.append(dict(symbol="BTCUSDT", direction=t.direction, gate=t.gate,
                         signal_day_idx=t.signal_day_idx, entry_time=t.entry_time,
                         exit_time=t.exit_time, entry_idx=t.entry_idx, exit_idx=t.exit_idx,
                         pnl=t.pnl, r=t.r_multiple, reason=t.reason, holding_bars=t.holding_bars,
                         risk_amount=t.risk_amount, hhi_z_at_signal=t.hhi_z_at_signal))
    if not rows:
        return pd.DataFrame(columns=["symbol", "direction", "gate", "signal_day_idx", "entry_time",
                                     "exit_time", "entry_idx", "exit_idx", "pnl", "r", "reason",
                                     "holding_bars", "risk_amount", "hhi_z_at_signal"])
    d = pd.DataFrame(rows)
...
def winrate_fixed_shuffle_test(r: np.ndarray, n_rep: int = 100, seed: int = 321) -> dict:
    """승률을 실제값으로 고정한 대안 검정: 실제 승/패 R값 풀을 유지한 채 순서(페어링)만
    무작위 재배치(즉 승 R 값 집합과 패 R 값 집합은 고정, 어느 트레이드가 승/패인지의 매핑만
    바뀌는게 아니라 — 실제로는 승패 라벨은 각 트레이드 고유이므로, 여기서는 승 R값들과 패 R값들의
    풀에서 원래 개수만큼 비복원추출 없이 그대로 사용하고 순서만 셔플해 평균은 원래와 동일해짐.
    따라서 이 그대로는 무의미 — 실제 유효한 대안은 '승 R 값 풀'과 '패 R 값 풀' 각각에서 개수를
    고정한 채 어느 트레이드가 승/패인지의 대응관계(=r 값 자체)를 셔플하지 않고, 대신 무작위
    이진 성패 시퀀스를 실제 승률로 생성한 뒤 승리 시 무작위로 승 R 풀에서, 패배 시 패 R 풀에서
    복원추출해 평균 재계산 — 승률 자체를 실제값에 고정한 귀무가설."""
    rng = np.random.default_rng(seed)
    n = len(r)
    if n == 0:
        return dict(pctile=float("nan"), actual_mean=float("nan"))
