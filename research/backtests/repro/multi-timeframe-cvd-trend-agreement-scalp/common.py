"""멀티TF CVD 추세 합치/불일치 필터 스캘프 — 공통 데이터 로더 + 지표.
...
SP = Path(os.environ.get(
    "MTFCVD_SCRATCH",
    "/tmp/claude-0/-home-user-study/5c4aa354-51d8-5c5e-afb3-edb007c0a106/scratchpad/mtfcvd"))
DATA = SP / "data"
...
def prefetch(symbols=SYMBOLS, tfs=("5m", "15m", "1h"),
...
def delta_bar(df: pd.DataFrame) -> pd.Series:
    """delta_bar = 2*taker_buy_base_volume - volume (매수 우세 양수)."""
    return 2.0 * df["taker_buy_volume"] - df["volume"]


def rolling_cvd(df: pd.DataFrame, window: int) -> pd.Series:
    """최근 window 개 봉의 delta_bar 롤링합(스펙 CVD_5m/CVD_1h 정의)."""
    return delta_bar(df).rolling(window, min_periods=window).sum()


def slope(series: pd.Series, lookback: int) -> pd.Series:
    """자기 자신의 직전 lookback 봉(현재봉 포함) 선형회귀 기울기(x=0..lookback-1)."""
    ...
    return series.rolling(lookback, min_periods=lookback).apply(_slope, raw=True)
