...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    rows = []
    for t in trades:
        rows.append(dict(symbol=t.symbol, mode=t.mode, direction=t.direction,
                         raw_direction=t.raw_direction, gate=t.gate,
                         signal_idx=t.signal_idx, confirm_idx=t.confirm_idx,
                         ...
                         ar_pctile_at_signal=t.ar_pctile_at_signal))
    if not rows:
        return pd.DataFrame(columns=["symbol", "mode", "direction", "raw_direction", "gate",
                                     "signal_idx", "confirm_idx", "entry_time",
                                     "exit_time", "entry_idx", "exit_idx", "pnl", "r", "reason",
                                     "holding_bars", "risk_amount", "ar_pctile_at_signal"])
    d = pd.DataFrame(rows)
