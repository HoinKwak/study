...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    rows = []
    for t in trades:
        rows.append(dict(symbol=t.symbol, direction=t.direction, dir_mode=t.dir_mode,
                         ...
                         z_at_signal=t.z_at_signal, px_ret_4h_at_signal=t.px_ret_4h_at_signal))
    if not rows:
        return pd.DataFrame(columns=["symbol", "direction", "dir_mode",
                                     "entry_time", "exit_time", "entry_idx", "exit_idx", "pnl",
                                     "r", "reason", "holding_bars", "risk_amount", "z_at_signal",
                                     "px_ret_4h_at_signal"])
    d = pd.DataFrame(rows)
...
def sign_shuffle_test(r: np.ndarray, n_rep: int = 100, seed: int = 123) -> dict:
    """부호 무작위화(50/50) 대조군 — 표본 수 불변, 100회. 각 트레이드의 방향 선택이 진짜 정보를
    담고 있는지, 무작위 방향 대비 실제 mean(R) 이 어느 백분위인지 계산(established convention,
    실제 반대방향 재시뮬레이션이 아니라 크기 고정·부호만 반전하는 근사 — engine 청산이 SL/RR-목표/
    z-회귀 모두 방향에 대해 대칭이라 근사 타당성 높음). 이 백테스트에서는 스펙이 요구한 "방향
    필터(직전4h수익률 부호)의 부가가치" 검정으로 사용한다."""
    rng = np.random.default_rng(seed)
    n = len(r)
    if n == 0:
