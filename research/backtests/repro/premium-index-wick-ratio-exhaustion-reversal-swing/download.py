"""데이터 수집: klines(4h, 1d) + premiumIndexKlines(4h) — 7종목, 2022-01~2026-06.

data.binance.vision 월간 덤프. fapi 는 이 컨테이너에서 451 지역차단이라 정적 덤프만 사용.
2022년 zip 은 헤더 없음, 2024년 이후는 헤더 있음(스펙 사전경고) — 헤더 유무를 자동판별.
"""
...
def main() -> None:
    for symbol in SYMBOLS:
        for kind, tf in [("klines", "4h"), ("klines", "1d"), ("premiumIndexKlines", "4h")]:
            fn = DATA / f"{symbol}_{kind}_{tf}.parquet"
            if fn.exists():
                print("skip", fn.name)
                continue
            print("fetch", symbol, kind, tf)
