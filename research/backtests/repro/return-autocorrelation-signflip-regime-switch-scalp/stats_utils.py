"""통계 진단 유틸 — PF(R), t검정, 부트스트랩, de-clustering, 셔플. (ATR-sync repro 패턴 계승)"""
...
def trades_df(trades: list[engine.TradeRec], rcol: str = "r_net") -> pd.DataFrame:
    ...
    for t in trades:
        rows.append(dict(symbol=t.symbol, mode=t.mode, direction=t.direction,
                         orig_direction=t.orig_direction,
    ...
    cols = ["symbol", "mode", "direction", "orig_direction", "entry_time", "exit_time", "entry_idx",
...
def fmt(d: dict) -> str:
    return (f"{d['label']:30s} n={d['n']:6d} PF(R)={d['pf_r']:.3f} mean(R)={d['mean_r']:+.4f} "
...
def bootstrap_diff_indep(r_a: np.ndarray, r_b: np.ndarray, n_boot: int = 5000, seed: int = 11
    ...
    """P(mean(A_boot)<=mean(B_boot)) — a가 b보다 우수한지(부트스트랩 p)."""
    rng = np.random.default_rng(seed)
