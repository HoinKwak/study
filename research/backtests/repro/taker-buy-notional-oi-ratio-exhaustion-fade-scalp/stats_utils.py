"""통계 진단 유틸 — PF(R), t검정, 부트스트랩, de-clustering. (oivolratio 라운드 템플릿 재사용)"""
...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    rows = []
    for t in trades:
        rows.append(dict(symbol=t.symbol, mode=t.mode, reverse=t.reverse, direction=t.direction,
                         orig_direction=t.orig_direction, entry_time=t.entry_time,
                         ...
                         risk_amount=t.risk_amount, z_trigger=t.z_trigger,
                         body_ratio_trigger=t.body_ratio_trigger, trail_used=t.trail_used))
    if not rows:
        return pd.DataFrame(columns=["symbol", "mode", "reverse", "direction", "orig_direction",
                                     "entry_time", "exit_time", "entry_idx", "exit_idx", "pnl",
                                     "r", "reason", "holding_bars", "risk_amount", "z_trigger",
                                     "body_ratio_trigger", "trail_used"])
    d = pd.DataFrame(rows)
