...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    rows = []
    for t in trades:
        rows.append(dict(symbol=t.symbol, direction=t.direction, signal_source=t.signal_source,
                         dir_mode=t.dir_mode, entry_time=t.entry_time, exit_time=t.exit_time,
    ...
    if not rows:
        return pd.DataFrame(columns=["symbol", "direction", "signal_source", "dir_mode",
...
def sign_shuffle_test(r: np.ndarray, n_rep: int = 100, seed: int = 123) -> dict:
    ...
    실제 반대방향 재시뮬레이션이 아니라 크기 고정·부호만 반전하는 근사 — engine 청산이 SL/트레일링
    ATR 폭에서 방향에 대해 대칭이라 근사 타당성 높음)."""
    rng = np.random.default_rng(seed)
    n = len(r)
    if n == 0:
