"""통계 진단 유틸 — PF(R), t검정, 부트스트랩, de-clustering. (기존 taker-buy-ratio-rolling-variance
백테스트 repro 의 동일 파일을 그대로 재사용 — 범용 유틸이라 신호 정의와 무관)"""
...
def trades_df(trades: list[engine.TradeRec]) -> pd.DataFrame:
    ...
    for t in trades:
        rows.append(dict(symbol=t.symbol, direction=t.direction, gate=t.gate,
                         ...
                         gap_pctile_at_signal=t.gap_pctile_at_signal,
    ...
    if not rows:
        return pd.DataFrame(columns=["symbol", "direction", "gate", "signal_idx", "entry_time",
                                     ...
                                     "holding_bars", "risk_amount", "gap_pctile_at_signal",
                                     "fee_entry", "fee_exit"])
    d = pd.DataFrame(rows)
