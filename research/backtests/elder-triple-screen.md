...
## 부록: backtest-reviewer 적대적 감사 (2026-07-18)

**판정: SUSPECT** (HOLD 유지 동의 — PASS/VALID 격상 불가, 라이브 미반영. INVALID는 아님).

감사자가 `simulate()`를 13종목 전체에 독립 재실행해 핵심 수치를 **소수점까지 재현**:
IS n=6522 PF=1.1971 t=2.340, OOS n=6051 PF=1.3141 t=3.807. 16조합 플래토(OOS PF
1.285~1.406)·leave-one-symbol-out(풀드 PF 1.219~1.270) 강건성도 재현. 룩어헤드(코드
라인추적 + 같은-봉 SL 변형실험 결과 미래누출 없음, 오히려 원코드가 더 보수적)·단위(ms
일치)·비용(왕복 0.14% 코드확인)·데이터소스(22종목 pkl 검증)는 모두 PASS.

**그러나 아래 4가지가 미검증이라 SUSPECT(라이브 미반영):**
1. **통계 유의성 과대추정** — 풀드 t는 12,593 트레이드를 독립표본 가정한 의사반복
   (pseudo-replication). 심볼 단위(n=13) 재계산 시 IS t=2.03 / OOS t=4.39(방향 유지,
   OOS는 오히려 강함).
2. **펀딩비 전무** — 평균 보유 37봉(1h)≈4.6 펀딩주기. 0.01%/8h 가정 시 트레이드당 gross
   엣지(0.293%/notional)의 15~20% 잠식 가능. 추세추종 특성상 강세장 롱 보유 중 펀딩
   스파이크 역상관 리스크.
3. **유니버스 브레스** — 헤드라인 "13/13(100%)"는 LONG_HISTORY(데이터길이 기준, 편향無)
   집계. 전체 22종목 기준 4개(BILL 0.51·HYPE 0.94·MRVL 0.66·VELVET 0.97) net PF<1.0로
   **18/22(82%)** 통과 — 본문에 수치 미명시.
4. **드로다운 미검토** — 고정자본 1% risk 사이징에서 XRP MDD 182.7%·DOGE 78.0%·ZEC 78.1%.
   실 마진/레버리지(10x/30x) 사이징 하 리스크 미검증.

**권장 후속검증(→ VALID/INVALID 판별)**: (a) 펀딩비 포함 재백테스트, (b) 심볼단위 t-검정
재보고, (c) 22종목 전체 breadth 본문 명시, (d) 실 마진/레버리지 사이징 하 드로다운 재평가.
