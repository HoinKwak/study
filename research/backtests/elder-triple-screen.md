...
> **⚠️ 후속 심층검증 결과(2026-07-18) — 최종 FAIL로 확정.** backtest-reviewer 1차 감사
> (SUSPECT, 4개 미검증 항목 지목) 후 펀딩비·심볼단위 t검정·22종목 브레스·**실 레버리지/
> 증거금 사이징**을 전부 검증했다(`research/backtests/elder-1h-breakout-atr-trailing-followup.md`).
> 펀딩비·통계적 유의성·브레스는 전부 통과했으나, **라이브 실제 사이징(증거금10%·시총상위
> 30x)을 코드 그대로 재현하자 13종목 동시운용 시 파산(MDD 100%, 트레이드의 6.5%가 이
> 전략의 SL보다 먼저 청산가에 도달)**했다 — 방향성 엣지는 유효해 보이나 이 전략의 ATR
> 기반 손절폭이 30x 레버리지 청산 버퍼(≈3.33%)보다 넓은 경우가 흔해 현재 라이브 사이징
> 설정과 근본적으로 양립 불가능하다. **최종 판정: FAIL(라이브 미반영)** — 아래 원 라운드4
> 분석(HOLD 판정)은 이 후속검증 이전 상태이므로 후속 리포트를 최종 근거로 삼을 것.
