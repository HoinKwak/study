...
## ⚠️ 대상 필터 — 크립토 네이티브 코인만 (최우선)
- **토큰화 주식·주식화 토큰·ETF·상품(commodity) perp는 전부 제외한다.** 예: SNDK(SanDisk)·
  SKHYNIX·MU(Micron)·NVDA·AAPL·TSLA·SPY·SOXL·SP500/지수·금(XAU/PAXG 상품형 제외 아님 주의:
  PAXG는 크립토화 금이지만 '주식화'는 아님 — 애매하면 제외)·은·원유 등 **TradFi 기초자산을
  토큰화한 것**은 CEX·DEX(HIP-3 xStocks 포함) 어디서 나오든 리스트에 넣지 않는다.
- **암호화폐 네이티브 자산만** 담는다(BTC·ETH·SOL·알트코인·밈코인·거버넌스/DeFi/L1·L2 토큰,
  Hyperliquid 자체 토큰 등). 판단 기준: "이게 원래 주식·ETF·상품이면 제외, 크립토면 포함."
- 시장 전반·테마에서 'TradFi/RWA perp가 상위 장악' 같은 맥락은 **한 줄로만 언급 가능**하되,
  cex/dex 종목 리스트에는 주식화 토큰을 넣지 않는다.
