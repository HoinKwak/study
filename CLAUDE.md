...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(7/31): RSI14 고정익손절·Ehlers ITL·장악형+RSI·핀바 키레벨(A/B 승자없음)·공적분 statarb 축소판 5종 모두 FAIL·VALID. 리뷰어가 방향반전 테스트(뒤집어도 더 악화)로 무엣지 확증, statarb에서 '진입수수료 pnl 미반영' 버그 재발 발견(패치 시 더 악화 — 판정불변, 멀티다리 전략은 raw-fees==pnl 정합 sanity 필수). 7/30: AC 투바 FAIL·VALID, 삼각형 FAIL·SUSPECT(스윙반경 재검 선택과제). **scalp15m이 여전히 유일 라이브 전략**. Andean Oscillator 페이퍼 후보 기록만.
