...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(8/3 오후): 딥스매수 고정%(A/B 모두 PF<1, 리뷰어 숏반전 실측 0.77~0.81 양방향 무엣지)·코랄 트렌드+ADX(OOS 1.086이나 1.3 미달·top-N 붕괴)·앵커드 VWAP 반응형(앵커 과리셋→σ밴드 과협소 59% 터치 구조 결함 — 리뷰어 재계산은 평균 3.57봉으로 결함 더 심함 확인) 3종 **FAIL·VALID**. 신규 스펙 4종 추가(코랄·딥스·돈치안 앙상블·캐리 합성, 총 226 — 뒤 2종은 포트폴리오 엔진 필요 보류). 8/3 오전: QuantPedia MACD D1/H1 FAIL·VALID(원문 정반대 재현), TSMOM 월간 FAIL·VALID(top-1 69.7% 단일 이벤트 의존). 8/2: z-score 계열 4종 최종 FAIL 확정, CUSUM·아시아세션·RSI DCA 래더 FAIL·VALID. **scalp15m이 여전히 유일 라이브 전략**. 미검증 대기: 극값 스트릭·볼린저 반대밴드·HMM 국면·VPIN·AlphaTrend·ICT 브레이커·52주 신고가/돈치안 앙상블/캐리 합성(포트폴리오 엔진 필요)·코인베이스 프리미엄·SSR(외부 데이터 필요) 등.
