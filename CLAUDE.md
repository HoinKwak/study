...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(8/1): 프리미엄인덱스 Z스코어 평균회귀 **FAIL·VALID**(premiumIndexKlines 덤프 확보 성공, 전 조합 PF 0.45~0.82, 리뷰어 방향반전 테스트로 양방향 무엣지 확증, OOS 수익%/MDD% 열은 연속복리 아티팩트로 정정주석), RSI 히든 다이버전스 **HOLD·VALID**(OOS 17건 표본미달, SL-갭 가드 스킵 61~63%는 정당한 보호장치임을 리뷰어가 무력화 실험으로 실증 — 제거 시 허위 PASS 생성). BTC 국면게이트 로테이션은 멀티심볼 포트폴리오 엔진 필요로 보류(요청 없이 만들지 않음). 7/31: RSI14·Ehlers ITL·장악형+RSI·핀바·statarb 5종 FAIL·VALID(statarb 진입수수료 버그 재발 — 멀티다리 raw-fees==pnl sanity 필수). 7/30: AC 투바 FAIL·VALID, 삼각형 FAIL·SUSPECT(스윙반경 재검 선택과제). **scalp15m이 여전히 유일 라이브 전략**. 미검증 대기 스펙: VPIN·AlphaTrend·ICT 브레이커·OI 크라우딩 등.
