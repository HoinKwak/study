...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(8/2): 아시아세션+런던돌파·RSI DCA 마틴게일 래더(30x 청산 46~59%)·OU 반감기 적응형 z-score 3종 **FAIL·VALID**. 리뷰어가 OU 방향반전(모멘텀 연장)에서 **fragile-positive 신호 발견**(OOS PF 1.10이나 top20 제거 시 0.78 붕괴·종목편차 0.70~1.53) — "z-극단 fade 폐기"는 확정, 모멘텀 변형은 별도 스펙으로 정식 스위트 필요(다음 라운드 후보). 신규 스펙 5종 추가(HMM 국면·로그회귀 리스크메트릭 등, 총 212). 8/1: 프리미엄인덱스 Z스코어 FAIL·VALID(방향반전 양방향 무엣지), RSI 히든 다이버전스 HOLD·VALID(표본미달, SL-갭 가드는 정당 — 제거 시 허위 PASS). 7/31: RSI14·Ehlers ITL·장악형+RSI·핀바·statarb 5종 FAIL·VALID(진입수수료 버그 재발 이력 — 멀티다리 raw-fees==pnl sanity 필수). **scalp15m이 여전히 유일 라이브 전략**. 미검증 대기: VPIN·AlphaTrend·ICT 브레이커·OI 크라우딩·OU 모멘텀 변형 등.
