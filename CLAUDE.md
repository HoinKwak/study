...
> (마지막 갱신: 2026-08-04, 12시 전략라운드 반영)
...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(8/4 12시 라운드): 합성청산클러스터 자석-돌파(7종목×IS/OOS 전구간 PF<1, 방향반전도 PF 0.79로 무엣지, OI-15m ms/us 단위버그 발견·수정)·상위트레이더 포지션비율 스마트머니 모멘텀(OOS 풀링 PF 1.058·top10제거 시 손실전환·파라미터스윕 전부 미달 — 단 반대극성인 기존 컨트래리언 간이비교가 OOS PF 1.387로 부수적 관심 유발, 정식 재검증 필요 별도 표기) 2종 **FAIL·VALID**. 신규 스펙 5종 추가(합성청산클러스터·상위트레이더비율·울프웨이브·ATR그리드·VA마이그레이션, 총 236). 8/4 새벽 라운드: BTC 디커플링 순추종(풀링 PF 0.84·반전 0.73 양방향 무엣지)·펀딩 부호전환+OI 가속(gross부터 손실, fundingRate·OI 5분 메트릭 캐시 신규 확보)·AlphaTrend(PF 1.155였으나 top-10 제거 시 1.02 붕괴) 3종 FAIL·VALID. 8/3 오후: 딥스매수 고정%·코랄 트렌드+ADX·앵커드 VWAP 반응형 3종 FAIL·VALID. 8/3 오전: QuantPedia MACD D1/H1 FAIL·VALID(원문 정반대 재현), TSMOM 월간 FAIL·VALID(top-1 69.7% 단일 이벤트 의존). 8/2: z-score 계열 4종 최종 FAIL 확정, CUSUM·아시아세션·RSI DCA 래더 FAIL·VALID. **scalp15m이 여전히 유일 라이브 전략**. 미검증 대기: 극값 스트릭·볼린저 반대밴드·HMM 국면·울프웨이브·ATR그리드·VA마이그레이션·VPIN·ICT 브레이커·52주 신고가/돈치안 앙상블/캐리 합성/왜도(포트폴리오 엔진 필요)·코인베이스 프리미엄·SSR(외부 데이터 필요)·`long-short-account-ratio-contrarian-swing`(정식 재검증 후보, 2022년 5%결측 구간 포함 IS 재설정 권고) 등. 참고: pandas 3.0.3은 `to_datetime(unit="ms")`가 `datetime64[ms]` 유지(ns 자동승격 아님) — `Timestamp.value`(항상 ns)와 섞으면 단위사고 재발 가능, 경계.
