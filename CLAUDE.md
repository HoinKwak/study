...
- **미션 #28(진행중)**: **15m/1h VALID 전략 발굴**. 지금까지 발굴·백테스트한 다수 전략 **전부 FAIL**(backtest-reviewer로 재검증해 거짓 FAIL 아님 확인). 최근(8/3): QuantPedia MACD D1/H1 **FAIL·VALID**(원문 단계개선 주장이 정반대 0.90→0.65로 재현 실패, 방향반전도 무엣지 — 이전 MACD 계열 FAIL과 교차 일치), TSMOM 월간 크로스에셋 **FAIL·VALID**(OOS 1.26이나 top-1 거래가 손익 69.7% — 2024-11 트럼프 랠리 단일 이벤트 의존, 룩백9 대안도 동일 결함. 크립토 BTC 고상관으로 원 논문 분산 전제 불성립 확증). 신규 스펙 5종 추가(MACD D1H1·앵커드VWAP·극값스트릭·볼린저 반대밴드·TSMOM, 총 222). 8/2: OU 모멘텀 연장 FAIL·VALID(**z-score 계열 4종 최종 FAIL 확정**), CUSUM+트리플배리어 FAIL·VALID(양방향 무엣지·복리 붕괴 아티팩트 발견→IS/OOS equity 리셋·고정명목 권고), 아시아세션 런던돌파·RSI DCA 래더·OU fade FAIL·VALID. **scalp15m이 여전히 유일 라이브 전략**. 미검증 대기: 앵커드 VWAP 반응형(차기 15m/1h 후보)·VPIN·AlphaTrend·ICT 브레이커·52주 신고가(포트폴리오 엔진 필요)·코인베이스 프리미엄·SSR(외부 데이터 필요) 등.
